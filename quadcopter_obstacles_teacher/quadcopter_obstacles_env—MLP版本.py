...
class QuadcopterObstaclesEnvCfg:
    ...
    num_dynamic_obstacles: int = 60
    dynamic_obstacle_max_amplitude: float = 1.2
    dynamic_obstacle_min_speed: float = 0.2
    dynamic_obstacle_max_speed: float = 0.6
    dynamic_obstacle_resample_interval_s: float = 5.0
    curriculum_enabled: bool = True
    curriculum_static_obstacle_pool: int = 200
    curriculum_dynamic_obstacle_pool: int = 60
    curriculum_stage0_static_obstacles: int = 100
    curriculum_stage0_dynamic_obstacles: int = 0
    curriculum_stage1_success_threshold: float = 0.55
    curriculum_stage1_static_obstacles: int = 140
    curriculum_stage1_dynamic_obstacles: int = 20
    curriculum_stage2_success_threshold: float = 0.70
    curriculum_stage2_static_obstacles: int = 180
    curriculum_stage2_dynamic_obstacles: int = 40
    curriculum_success_ema_alpha: float = 0.05
    curriculum_min_stage_steps: int = 48000
    ...
    target_velocity_reward_scale: float = 2.0
    ...
    time_to_collision_reward_scale: float = -5.0
    # 平滑：抑制相邻控制指令突变。
    action_smoothness_reward_scale: float = -0.10
    ...
    def policy_observation_dim(self) -> int:
        ...
        base_dim = 3 + 3 + 3 + 3 + 3
        closest_obstacle_dim = self.num_closest_obstacles * 3 + self.num_closest_obstacles + self.num_closest_obstacles * 3
        extra_dim = 1 + 1 + 1
    ...
    def __post_init__(self):
        ...
        if self.curriculum_enabled:
            self.num_obstacles = max(self.num_obstacles, self.curriculum_static_obstacle_pool + self.curriculum_dynamic_obstacle_pool)
            self.num_dynamic_obstacles = max(self.num_dynamic_obstacles, self.curriculum_dynamic_obstacle_pool)
...
class QuadcopterObstaclesEnv(gym.Env):
    ...
    def __init__(self, cfg: QuadcopterObstaclesEnvCfg | None = None, render_mode: str | None = None, **kwargs):
        ...
        self._dynamic_time_s = 0.0
        self._dynamic_next_resample_time_s = 0.0
        ...
        self._dynamic_obstacle_axis_xy = torch.zeros((self.cfg.num_obstacles, 2), device=self.device)
        self._dynamic_obstacle_xy_amplitude = torch.zeros(self.cfg.num_obstacles, device=self.device)
        ...
        self._dynamic_obstacle_angular_speed = torch.zeros(self.cfg.num_obstacles, device=self.device)
        self._dynamic_obstacle_phase = torch.zeros(self.cfg.num_obstacles, device=self.device)
        ...
        self._curriculum_last_stage_step = 0
    ...
    def _build(self) -> None:
        ...
        print(
            ...
            f"(stage={self._curriculum_stage}, dynamic={self._current_dynamic_obstacle_count})"
        )
    ...
    def _curriculum_stage_counts(self, stage_idx: int) -> tuple[int, int]:
        stage_idx = int(stage_idx)
        static_counts = (
            int(self.cfg.curriculum_stage0_static_obstacles),
            int(self.cfg.curriculum_stage1_static_obstacles),
            int(self.cfg.curriculum_stage2_static_obstacles),
        )
        dynamic_counts = (
            int(self.cfg.curriculum_stage0_dynamic_obstacles),
            int(self.cfg.curriculum_stage1_dynamic_obstacles),
            int(self.cfg.curriculum_stage2_dynamic_obstacles),
        )
        stage_idx = max(0, min(stage_idx, len(static_counts) - 1))
        return static_counts[stage_idx], dynamic_counts[stage_idx]

    def _apply_obstacle_curriculum_layout(self, sync_prims: bool = True) -> None:
        self._dynamic_time_s = 0.0
        self._dynamic_next_resample_time_s = float(self.cfg.dynamic_obstacle_resample_interval_s)
        self._dynamic_obstacle_axis_xy.zero_()
        self._dynamic_obstacle_xy_amplitude.zero_()
        self._dynamic_obstacle_speed.zero_()
        self._dynamic_obstacle_velocity_xy.zero_()
        self._dynamic_obstacle_angular_speed.zero_()
        self._dynamic_obstacle_phase.zero_()
        self._active_obstacle_mask.zero_()
        self._dynamic_obstacle_mask.zero_()

        static_count, dynamic_count = self._curriculum_stage_counts(self._curriculum_stage)
        static_pool = int(self.cfg.curriculum_static_obstacle_pool)
        dynamic_pool = int(self.cfg.curriculum_dynamic_obstacle_pool)
        static_count = min(static_count, static_pool)
        dynamic_count = min(dynamic_count, dynamic_pool)

        static_indices = torch.arange(static_count, device=self.device, dtype=torch.long)
        dynamic_start = static_pool
        dynamic_indices = torch.arange(dynamic_start, dynamic_start + dynamic_count, device=self.device, dtype=torch.long)
    ...
    def _maybe_advance_curriculum(self) -> bool:
        ...
        thresholds = (float(self.cfg.curriculum_stage1_success_threshold), float(self.cfg.curriculum_stage2_success_threshold))
        if self._curriculum_stage >= len(thresholds):
            return False
        if (self.common_step_counter - self._curriculum_last_stage_step) < int(self.cfg.curriculum_min_stage_steps):
            return False
        if self._curriculum_success_ema < thresholds[self._curriculum_stage]:
            return False

        self._curriculum_stage += 1
        self._curriculum_last_stage_step = self.common_step_counter
        self._apply_obstacle_curriculum_layout(sync_prims=True)
        print(
            f"[INFO] Curriculum advanced to stage {self._curriculum_stage}: "
            f"static={self._current_static_obstacle_count}, dynamic={self._current_dynamic_obstacle_count}, "
            f"success_ema={self._curriculum_success_ema:.3f}"
        )
        return True
    ...
    def _resample_dynamic_obstacle_velocities(self, obstacle_indices: torch.Tensor | None = None) -> None:
        ...
        interval = float(self.cfg.dynamic_obstacle_resample_interval_s)
        for obstacle_idx in obstacle_indices.tolist():
            pos_xy = self._obstacle_positions_w[obstacle_idx, :2]
            limit = self.cfg.obstacle_spawn_range - self._obstacle_radii[obstacle_idx]
            chosen_velocity = None
            for _ in range(64):
                angle = torch.empty(1, device=self.device).uniform_(0.0, 2.0 * torch.pi).item()
                direction = torch.tensor([np.cos(angle), np.sin(angle)], device=self.device, dtype=self._obstacle_positions_w.dtype)
                speed = torch.empty(1, device=self.device).uniform_(
                    self.cfg.dynamic_obstacle_min_speed, self.cfg.dynamic_obstacle_max_speed
                ).item()
                candidate_velocity = direction * speed
                future_xy = pos_xy + candidate_velocity * interval
                if bool((torch.abs(future_xy) <= limit).all()):
                    chosen_velocity = candidate_velocity
                    self._dynamic_obstacle_speed[obstacle_idx] = speed
                    break

            if chosen_velocity is None:
                center_direction = -pos_xy
                center_norm = torch.linalg.norm(center_direction).item()
                if center_norm < 1e-6:
                    fallback_angle = torch.empty(1, device=self.device).uniform_(0.0, 2.0 * torch.pi).item()
                    center_direction = torch.tensor([np.cos(fallback_angle), np.sin(fallback_angle)], device=self.device, dtype=self._obstacle_positions_w.dtype)
                else:
                    center_direction = center_direction / center_norm
                speed = torch.empty(1, device=self.device).uniform_(
                    self.cfg.dynamic_obstacle_min_speed, self.cfg.dynamic_obstacle_max_speed
                ).item()
                chosen_velocity = center_direction * speed
                self._dynamic_obstacle_speed[obstacle_idx] = speed

            self._dynamic_obstacle_velocity_xy[obstacle_idx] = chosen_velocity

    def _initialize_dynamic_obstacles(self, base_positions_w: torch.Tensor) -> torch.Tensor:
        # 动态障碍物只在水平面内做匀速直线运动，并每隔固定时长重采样速度方向和大小。
        self._dynamic_time_s = 0.0
        self._dynamic_next_resample_time_s = float(self.cfg.dynamic_obstacle_resample_interval_s)
        self._obstacle_base_positions_w[:] = base_positions_w
        self._obstacle_positions_w[:] = base_positions_w
        self._dynamic_obstacle_mask.zero_()
        self._dynamic_obstacle_axis_xy.zero_()
        self._dynamic_obstacle_xy_amplitude.zero_()
        self._dynamic_obstacle_speed.zero_()
        ...
        if (not self.cfg.dynamic_obstacles_enabled) or self.cfg.num_obstacles == 0:
            return self._obstacle_positions_w

        requested_dynamic = min(max(int(self.cfg.num_dynamic_obstacles), 0), self.cfg.num_obstacles)
        if requested_dynamic == 0:
            return self._obstacle_positions_w

        center_clearance = torch.linalg.norm(base_positions_w[:, :2], dim=1) - (
            self.cfg.obstacle_safe_zone + self._obstacle_radii + 0.2
        )
        boundary_clearance = self.cfg.obstacle_spawn_range - (torch.abs(base_positions_w[:, :2]).amax(dim=1) + self._obstacle_radii)
        motion_clearance = torch.minimum(center_clearance, boundary_clearance)
        eligible_indices = torch.nonzero(motion_clearance > 0.35, as_tuple=False).squeeze(-1)
        if eligible_indices.numel() == 0:
            return self._obstacle_positions_w

        if eligible_indices.numel() > requested_dynamic:
            perm = torch.randperm(eligible_indices.numel(), device=self.device)[:requested_dynamic]
            dynamic_indices = eligible_indices[perm]
        else:
            dynamic_indices = eligible_indices
    ...
    def _update_dynamic_obstacles(self, dt: float = 0.0) -> None:
        ...
        while remaining_dt > 1e-8:
            time_until_resample = self._dynamic_next_resample_time_s - self._dynamic_time_s
            step_dt = min(remaining_dt, max(time_until_resample, 0.0))
            if step_dt > 0.0:
                self._obstacle_positions_w[dynamic_indices, :2] += self._dynamic_obstacle_velocity_xy[dynamic_indices] * step_dt
                ...
                self._dynamic_time_s += step_dt
                remaining_dt -= step_dt

            if self._dynamic_time_s + 1e-8 >= self._dynamic_next_resample_time_s:
                self._resample_dynamic_obstacle_velocities(dynamic_indices)
                self._dynamic_next_resample_time_s += float(self.cfg.dynamic_obstacle_resample_interval_s)
            elif step_dt <= 0.0:
                break
    ...
    def _spawn_shared_obstacles(self) -> None:
        ...
        self._apply_obstacle_curriculum_layout(sync_prims=False)
        self._obstacle_paths.clear()
        self._obstacle_translate_ops.clear()
        dynamic_start = int(self.cfg.curriculum_static_obstacle_pool)
    ...
    def _compute_closest_obstacle_relative_velocity_body(self, drone_quat_w: torch.Tensor) -> torch.Tensor:
        # 为最近若干障碍物提供相对速度观测，帮助策略显式感知“障碍物正在往哪边动”。
        ...
        closest_obstacle_velocities_w = obstacle_velocities_w[closest_indices]
        relative_velocities_w = closest_obstacle_velocities_w - drone_lin_vel_w.unsqueeze(1)

        num_closest = self.cfg.num_closest_obstacles
        relative_velocities_flat = relative_velocities_w.reshape(self.num_envs * num_closest, 3)
        drone_quat_expanded = drone_quat_w.unsqueeze(1).expand(-1, num_closest, -1).reshape(self.num_envs * num_closest, 4)
        relative_velocities_b = quat_apply_inverse(drone_quat_expanded, relative_velocities_flat).reshape(
            self.num_envs, num_closest, 3
        )

        relative_velocity_scale = max(
            self.cfg.cmd_body_vel_xy_max + self.cfg.dynamic_obstacle_max_speed,
            self.cfg.cmd_vel_z_max,
            1.0,
        )
        return (relative_velocities_b / relative_velocity_scale).clamp(-2.0, 2.0)
    ...
    def _get_observations(self) -> dict[str, torch.Tensor]:
        ...
        obstacle_relative_velocities_b = self._compute_closest_obstacle_relative_velocity_body(root_quat_w)
        ...
        # 最终把所有低维量拼成一个大向量：
        # [自身速度, 自身角速度, 姿态, 目标相对位置, 当前速度命令, 最近障碍物方向/距离/相对速度, 前向障碍距离, 目标方向障碍距离, target_reached]
        obs = torch.cat(
            [
                root_lin_vel_b,
                root_ang_vel_b,
                ...
                obstacle_distances,
                obstacle_relative_velocities_b.reshape(self.num_envs, -1),
    ...
    def step(self, actions: torch.Tensor):
        ...
        # 7. 动态风险惩罚：若最近动态障碍与无人机处于“相向闭合”状态，则提前惩罚。
        ...
        _, closest_indices = torch.topk(distances_sq_all, self.cfg.num_closest_obstacles, dim=1, largest=False)
        obstacle_velocities_w = torch.zeros((self.cfg.num_obstacles, 3), device=self.device)
        obstacle_velocities_w[:, :2] = self._dynamic_obstacle_velocity_xy
        batch_indices = torch.arange(self.num_envs, device=self.device).unsqueeze(1).expand(-1, self.cfg.num_closest_obstacles)
        closest_vectors_w = to_obstacles_w[batch_indices, closest_indices]
        closest_center_distances = torch.sqrt(distances_sq_all[batch_indices, closest_indices].clamp_min(1e-12))
        closest_surface_distances = (closest_center_distances - self._obstacle_radii[closest_indices]).clamp_min(1e-3)
        closest_directions_w = closest_vectors_w / closest_center_distances.unsqueeze(-1).clamp_min(1e-6)
        closest_relative_velocities_w = obstacle_velocities_w[closest_indices] - self.robot.data.root_lin_vel_w.unsqueeze(1)
        closing_speed = -(closest_relative_velocities_w * closest_directions_w).sum(dim=2)
        time_to_collision = closest_surface_distances / closing_speed.clamp_min(1e-3)
        dynamic_close_mask = self._dynamic_obstacle_mask[closest_indices]
        ttc_mask = dynamic_close_mask & (closing_speed > 0.05) & (time_to_collision < 3.0)
        ...
        if self.common_step_counter % 500 == 0:
            ...
            print(
                f"[DEBUG] Step {self.common_step_counter}: stage={self._curriculum_stage}, "
        ...
        if done_ids.numel() > 0:
            ...
            self._curriculum_success_ema = (1.0 - alpha) * self._curriculum_success_ema + alpha * batch_success_rate
            ...
            log["Metrics/curriculum_success_ema"] = float(self._curriculum_success_ema)
            log["Metrics/curriculum_active_obstacles"] = float(self._current_total_obstacle_count)
            log["Metrics/curriculum_dynamic_obstacles"] = float(self._current_dynamic_obstacle_count)

            curriculum_advanced = self._maybe_advance_curriculum()
