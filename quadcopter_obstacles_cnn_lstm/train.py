...
def _resolve_resume_checkpoint(log_root: str, run_name: str | None, checkpoint_name: str | None) -> str:
    """Resolve resume checkpoints from either an explicit file path or the legacy run+regex flow."""
    if checkpoint_name:
        checkpoint_path = os.path.abspath(checkpoint_name)
        if os.path.isfile(checkpoint_path):
            return checkpoint_path

    if not run_name or not checkpoint_name:
        raise ValueError(
            "Resuming training requires either an existing checkpoint file path via --checkpoint "
            "or both --load_run and --checkpoint for regex-based lookup."
        )

    return _find_checkpoint(log_root, run_name, checkpoint_name)
...
def main():
    ...
    # Recurrent PPO requires num_envs >= num_mini_batches.
    # For quick debugging with very small num_envs (e.g., 1), auto-cap mini-batches.
    if args_cli.num_envs is not None:
        capped_ppo_mbs = max(1, min(int(args_cli.num_envs), int(agent_cfg.algorithm.num_mini_batches)))
        if capped_ppo_mbs != int(agent_cfg.algorithm.num_mini_batches):
            print(
                f"[INFO] Auto-adjust PPO num_mini_batches: {agent_cfg.algorithm.num_mini_batches} -> {capped_ppo_mbs} \
for num_envs={args_cli.num_envs}",
                flush=True,
            )
            agent_cfg.algorithm.num_mini_batches = capped_ppo_mbs

        if hasattr(agent_cfg, "distill_num_mini_batches"):
            capped_distill_mbs = max(1, min(int(args_cli.num_envs), int(agent_cfg.distill_num_mini_batches)))
            if capped_distill_mbs != int(agent_cfg.distill_num_mini_batches):
                print(
                    f"[INFO] Auto-adjust distill_num_mini_batches: {agent_cfg.distill_num_mini_batches} -> {capped_distill_mbs} \
for num_envs={args_cli.num_envs}",
                    flush=True,
                )
                agent_cfg.distill_num_mini_batches = capped_distill_mbs
    if args_cli.seed is not None:
        agent_cfg.seed = args_cli.seed
    # Forward CLI debug flag into runner cfg so learn() can emit heartbeat logs.
    setattr(agent_cfg, "debug_logs", args_cli.debug_logs)
    ...
    direct_checkpoint_path = None
    if args_cli.checkpoint:
        candidate_checkpoint = os.path.abspath(args_cli.checkpoint)
        if os.path.isfile(candidate_checkpoint):
            direct_checkpoint_path = candidate_checkpoint

    if args_cli.experiment_name:
        agent_cfg.experiment_name = args_cli.experiment_name
    if args_cli.resume or direct_checkpoint_path is not None:
    ...
    if agent_cfg.resume:
        resume_path = _resolve_resume_checkpoint(log_root_path, agent_cfg.load_run, agent_cfg.load_checkpoint)
