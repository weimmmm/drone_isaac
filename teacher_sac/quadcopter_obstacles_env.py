...
class QuadcopterObstaclesEnvCfg:
    ...
    dynamic_obstacle_velocity_range: tuple[float, float] = (0.5, 1.5)
    ...
    dynamic_obstacle_collision_margin: float = 0.30
    dynamic_obstacle_proximity_reward_scale: float = -12.0
...
class QuadcopterObstaclesEnv(gym.Env):
    ...
    def __init__(self, cfg: QuadcopterObstaclesEnvCfg | None = None, render_mode: str | None = None, **kwargs):
        ...
        self._dynamic_obstacle_category_count = 8
        self._dynamic_obstacles_per_category = int(self.cfg.num_dynamic_obstacles / self._dynamic_obstacle_category_count)
        self._num_dynamic_obstacles = self._dynamic_obstacles_per_category * self._dynamic_obstacle_category_count
        self._dynamic_obstacle_width_resolution = (
            self.cfg.dynamic_obstacle_max_width / 4.0 if self._num_dynamic_obstacles > 0 else 1.0
        )
        self._dynamic_obstacle_positions_w = torch.zeros((self._num_dynamic_obstacles, 3), device=self.device)
        self._dynamic_obstacle_origins_w = torch.zeros_like(self._dynamic_obstacle_positions_w)
        self._dynamic_obstacle_goals_w = torch.zeros_like(self._dynamic_obstacle_positions_w)
        self._dynamic_obstacle_velocities_w = torch.zeros_like(self._dynamic_obstacle_positions_w)
    ...
    def _sample_dynamic_obstacle_origins(self) -> torch.Tensor:
        origins = torch.zeros((self._num_dynamic_obstacles, 3), device=self.device)
        if self._num_dynamic_obstacles == 0:
            return origins

        preferred_distance = 2.0 * np.sqrt(
            (self.cfg.map_half_extent * self.cfg.map_half_extent) / max(self._num_dynamic_obstacles, 1)
        )
        placed_xy: list[np.ndarray] = []
        cuboid_category_count = self._dynamic_obstacle_category_count // 2
        for category_idx in range(self._dynamic_obstacle_category_count):
            for local_idx in range(self._dynamic_obstacles_per_category):
                obstacle_idx = category_idx * self._dynamic_obstacles_per_category + local_idx
                current_min_distance = preferred_distance
                chosen_xy = None
                for attempt in range(128):
                    candidate_xy = np.random.uniform(
                        low=-self.cfg.map_half_extent,
                        high=self.cfg.map_half_extent,
                        size=2,
                    )
                    if all(np.linalg.norm(candidate_xy - prev_xy) > current_min_distance for prev_xy in placed_xy):
                        chosen_xy = candidate_xy
                        break
                    if attempt > 0 and attempt % 32 == 0:
                        current_min_distance *= 0.8
                if chosen_xy is None:
                    chosen_xy = np.random.uniform(
                        low=-self.cfg.map_half_extent,
                        high=self.cfg.map_half_extent,
                        size=2,
                    )

                placed_xy.append(chosen_xy)
                origins[obstacle_idx, 0] = float(chosen_xy[0])
                origins[obstacle_idx, 1] = float(chosen_xy[1])
                if category_idx < cuboid_category_count:
                    origins[obstacle_idx, 2] = torch.empty((), device=self.device).uniform_(
                        0.0, self.cfg.flight_max_height
                    )
                else:
                    origins[obstacle_idx, 2] = self.cfg.dynamic_obstacle_2d_height * 0.5
        return origins

    def _get_stage(self):
        if hasattr(self.sim, "stage"):
            return self.sim.stage
        import omni.usd

        return omni.usd.get_context().get_stage()

    def _cache_dynamic_obstacle_translate_op(self, prim_path: str):
    ...
    def _set_dynamic_obstacle_visual_position(self, idx: int, position: torch.Tensor) -> None:
        if idx >= len(self._dynamic_obstacle_translate_ops):
            return
        pos = position.detach().cpu().tolist()
        self._dynamic_obstacle_translate_ops[idx].Set(Gf.Vec3d(float(pos[0]), float(pos[1]), float(pos[2])))
    ...
    def _spawn_dynamic_obstacles(self) -> None:
        ...
        cuboid_category_count = self._dynamic_obstacle_category_count // 2
        for category_idx in range(self._dynamic_obstacle_category_count):
            width = float((category_idx % cuboid_category_count) + 1) * self._dynamic_obstacle_width_resolution
            is_cuboid = category_idx < cuboid_category_count
            height = self.cfg.dynamic_obstacle_3d_height if is_cuboid else self.cfg.dynamic_obstacle_2d_height
            start_idx = category_idx * self._dynamic_obstacles_per_category
            end_idx = start_idx + self._dynamic_obstacles_per_category
            self._dynamic_obstacle_sizes[start_idx:end_idx] = torch.tensor(
                [width, width, height], dtype=torch.float, device=self.device
            )

            for obstacle_idx in range(start_idx, end_idx):
                prim_path = f"/World/DynamicObstacles/Obstacle_{obstacle_idx:03d}"
                if is_cuboid:
                    obstacle_cfg = sim_utils.CuboidCfg(
                        size=(width, width, height),
                        collision_props=sim_utils.CollisionPropertiesCfg(collision_enabled=False),
                        visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=(0.0, 0.85, 0.25), metallic=0.1),
                    )
                else:
                ...
                obstacle_cfg.func(
                    prim_path,
                    obstacle_cfg,
                    translation=tuple(self._dynamic_obstacle_positions_w[obstacle_idx].tolist()),
                )
                self._dynamic_obstacle_paths.append(prim_path)
                self._dynamic_obstacle_translate_ops.append(self._cache_dynamic_obstacle_translate_op(prim_path))
    ...
    def _move_dynamic_obstacles(self) -> None:
        ...
        goal_distance = torch.linalg.norm(
            self._dynamic_obstacle_positions_w - self._dynamic_obstacle_goals_w,
            dim=1,
        )
        new_goal_mask = (goal_distance < self.cfg.dynamic_obstacle_goal_threshold) | (
            self._dynamic_obstacle_step_count == 0
        )
        num_new_goals = int(torch.count_nonzero(new_goal_mask).item())
        if num_new_goals > 0:
            local_range = torch.tensor(self.cfg.dynamic_obstacle_local_range, device=self.device)
            local_samples = -local_range + 2.0 * local_range * torch.rand((num_new_goals, 3), device=self.device)
            self._dynamic_obstacle_goals_w[new_goal_mask] = self._dynamic_obstacle_origins_w[new_goal_mask] + local_samples
            self._dynamic_obstacle_goals_w[:, 0].clamp_(-self.cfg.map_half_extent, self.cfg.map_half_extent)
            self._dynamic_obstacle_goals_w[:, 1].clamp_(-self.cfg.map_half_extent, self.cfg.map_half_extent)
            self._dynamic_obstacle_goals_w[:, 2].clamp_(0.0, self.cfg.flight_max_height)
            self._dynamic_obstacle_goals_w[self._num_dynamic_obstacles // 2 :, 2] = (
                self.cfg.dynamic_obstacle_2d_height * 0.5
            )

        resample_steps = max(1, int(round(self.cfg.dynamic_obstacle_velocity_resample_s / self.step_dt)))
        if self._dynamic_obstacle_step_count % resample_steps == 0:
            min_vel, max_vel = self.cfg.dynamic_obstacle_velocity_range
            velocity_norm = min_vel + (max_vel - min_vel) * torch.rand(
                (self._num_dynamic_obstacles, 1), device=self.device
            )
            direction = self._dynamic_obstacle_goals_w - self._dynamic_obstacle_positions_w
            self._dynamic_obstacle_velocities_w[:] = velocity_norm * direction / torch.linalg.norm(
                direction, dim=1, keepdim=True
            ).clamp_min(1e-6)

        self._dynamic_obstacle_positions_w += self._dynamic_obstacle_velocities_w * self.step_dt
        self._dynamic_obstacle_step_count += 1
        for idx in range(self._num_dynamic_obstacles):
            self._set_dynamic_obstacle_visual_position(idx, self._dynamic_obstacle_positions_w[idx])

    def _compute_dynamic_obstacle_signed_distance(self, drone_pos_w: torch.Tensor) -> torch.Tensor:
        if self._num_dynamic_obstacles == 0:
            return torch.full((self.num_envs,), float("inf"), device=self.device)

        rel_xy = drone_pos_w[:, None, :2] - self._dynamic_obstacle_positions_w[None, :, :2]
        rel_z = torch.abs(drone_pos_w[:, None, 2] - self._dynamic_obstacle_positions_w[None, :, 2])
        radii = self._dynamic_obstacle_sizes[:, 0] * 0.5
        heights = self._dynamic_obstacle_sizes[:, 2]
        radial = torch.linalg.norm(rel_xy, dim=2) - radii[None, :]
        vertical = rel_z - heights[None, :] * 0.5
        outside = torch.stack((radial, vertical), dim=-1).clamp_min(0.0)
        inside = torch.maximum(radial, vertical).clamp_max(0.0)
        signed_distance = torch.linalg.norm(outside, dim=-1) + inside
        return signed_distance.min(dim=1).values
    ...
    def _compute_dynamic_obstacle_features(
        ...
        if self._num_dynamic_obstacles == 0 or self.cfg.num_closest_dynamic_obstacles == 0:
            return (
                torch.zeros(feature_shape, device=self.device),
                torch.full((self.num_envs,), float("inf"), device=self.device),
                torch.zeros(self.num_envs, dtype=torch.bool, device=self.device),
            )

        num_closest = min(self.cfg.num_closest_dynamic_obstacles, self._num_dynamic_obstacles)
        rel_pos_w = self._dynamic_obstacle_positions_w.unsqueeze(0) - drone_pos_w.unsqueeze(1)
        rel_pos_for_sort = rel_pos_w.clone()
        rel_pos_for_sort[:, self._num_dynamic_obstacles // 2 :, 2] = 0.0
        distances_2d = torch.linalg.norm(rel_pos_for_sort[:, :, :2], dim=2)
        _, closest_indices = torch.topk(distances_2d, num_closest, dim=1, largest=False)
        batch_indices = torch.arange(self.num_envs, device=self.device).unsqueeze(1).expand(-1, num_closest)
        closest_rel_pos_w = rel_pos_w[batch_indices, closest_indices]
        closest_sort_rel_pos_w = rel_pos_for_sort[batch_indices, closest_indices]
        closest_sizes = self._dynamic_obstacle_sizes[closest_indices]
        closest_vel_w = self._dynamic_obstacle_velocities_w[closest_indices]

        out_of_range = torch.linalg.norm(closest_sort_rel_pos_w[:, :, :2], dim=2) > self.cfg.obstacle_detection_range
        closest_rel_pos_norm = closest_rel_pos_w / torch.linalg.norm(
            closest_rel_pos_w, dim=2, keepdim=True
        ).clamp_min(1e-6)

        quat_expanded = drone_quat_w.unsqueeze(1).expand(-1, num_closest, -1).reshape(self.num_envs * num_closest, 4)
        rel_pos_b = quat_apply_inverse(quat_expanded, closest_rel_pos_norm.reshape(self.num_envs * num_closest, 3))
        rel_pos_b = rel_pos_b.reshape(self.num_envs, num_closest, 3)
        vel_b = quat_apply_inverse(quat_expanded, closest_vel_w.reshape(self.num_envs * num_closest, 3))
        vel_b = vel_b.reshape(self.num_envs, num_closest, 3)
        max_dynamic_velocity = max(self.cfg.dynamic_obstacle_velocity_range[1], 1e-6)
        vel_b = vel_b / max_dynamic_velocity

        distance_xy = torch.linalg.norm(closest_sort_rel_pos_w[:, :, :2], dim=2, keepdim=True)
        distance_z = closest_rel_pos_w[:, :, 2:3]
        width_category = closest_sizes[:, :, 0:1] / self._dynamic_obstacle_width_resolution - 1.0
        height_category = torch.where(
            closest_sizes[:, :, 2:3] > self.cfg.dynamic_obstacle_3d_height,
            torch.zeros_like(closest_sizes[:, :, 2:3]),
            closest_sizes[:, :, 2:3],
        )
        ...
        features = torch.cat(
            [
                rel_pos_b,
                (distance_xy / self.cfg.obstacle_detection_range).clamp(0.0, 1.0),
                (distance_z / self.cfg.obstacle_detection_range).clamp(-1.0, 1.0),
                vel_b,
                width_category,
                height_category,
            ],
        ...
        features[out_of_range] = 0.0

        if num_closest < self.cfg.num_closest_dynamic_obstacles:
            padded = torch.zeros(feature_shape, device=self.device)
            padded[:, :num_closest] = features
            features = padded

        radii = closest_sizes[:, :, 0] * 0.5
        heights = closest_sizes[:, :, 2]
        dynamic_collision_2d = torch.linalg.norm(closest_sort_rel_pos_w[:, :, :2], dim=2) <= (
            radii + self.cfg.dynamic_obstacle_collision_margin
        )
        dynamic_collision_z = torch.abs(closest_rel_pos_w[:, :, 2]) <= (
            heights * 0.5 + self.cfg.dynamic_obstacle_collision_margin
        )
        dynamic_collision_each = dynamic_collision_2d & dynamic_collision_z & (~out_of_range)
        dynamic_collision = torch.any(dynamic_collision_each, dim=1)
        closest_dynamic_distance = self._compute_dynamic_obstacle_signed_distance(drone_pos_w)
        return features, closest_dynamic_distance, dynamic_collision
    ...
    def _get_observations(self) -> dict[str, torch.Tensor]:
        ...
        dynamic_obstacle_features, _, _ = self._compute_dynamic_obstacle_features(root_pos_w, root_quat_w)
    ...
    def step(self, actions: torch.Tensor):
        ...
        _, closest_dynamic_obstacle_distance, dynamic_obstacle_collision = self._compute_dynamic_obstacle_features(
        ...
        closest_any_hazard_distance = torch.minimum(closest_hazard_distance, closest_dynamic_obstacle_distance)
        ...
        dynamic_obstacle_proximity = torch.where(
            closest_dynamic_obstacle_distance < self.cfg.obstacle_proximity_trigger_distance,
            torch.exp(-closest_dynamic_obstacle_distance * 3.0),
            torch.zeros_like(closest_dynamic_obstacle_distance),
        )
        ...
        rewards = {
            ...
            "dynamic_obstacle_proximity": dynamic_obstacle_proximity
            * self.cfg.dynamic_obstacle_proximity_reward_scale
            * self.step_dt,
        ...
        died = too_low | too_high | obstacle_collision | dynamic_obstacle_collision | wall_collision
        ...
        if done_ids.numel() > 0:
            ...
            log["Metrics/avg_closest_hazard_distance"] = closest_any_hazard_distance.mean().item()
            log["Metrics/avg_closest_dynamic_obstacle_distance"] = closest_dynamic_obstacle_distance.mean().item()
            extras["log"] = log
