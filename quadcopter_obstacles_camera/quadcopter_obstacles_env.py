...
class QuadcopterObstaclesEnvCfg:
    ...
    depth_camera_horizontal_fov_deg: float = 90.0
    depth_camera_vertical_fov_deg: float = 60.0
    depth_camera_min_range: float = 0.1
    ...
    def __post_init__(self):
        ...
        self.policy_state_space = self.policy_observation_dim()
        self.observation_space = self.policy_state_space + self.depth_image_height * self.depth_image_width
        self.state_space = self.privileged_observation_dim()
...
class QuadcopterObstaclesEnv(gym.Env):
    ...
    def _build(self) -> None:
        ...
        for _ in range(5):
            ...
            if self._depth_camera is not None:
                self._depth_camera.update(self.cfg.physics_dt)

        print(
            f"[INFO] Quadcopter Obstacles Camera - Single World with {self.num_envs} robots, "
            f"{self.cfg.num_obstacles} static obstacles, and {self._num_dynamic_obstacles} dynamic obstacles"
        )
        print(
            "[INFO] Observation space: "
            f"policy_state={self.cfg.policy_state_space}, "
            f"policy_image=(1,{self.cfg.depth_image_height},{self.cfg.depth_image_width}), "
            f"flat={self.cfg.observation_space}"
        )
        print(f"[INFO] OmniNxt mass from asset: {self._robot_mass:.4f} kg")
        print(f"[INFO] Real depth camera: {self.cfg.depth_image_width}x{self.cfg.depth_image_height}")
    ...
    def _move_dynamic_obstacles(self) -> None:
        ...
        elif visual_update_interval > 0 and self._dynamic_obstacle_step_count % visual_update_interval == 0:
            self._update_dynamic_obstacle_visuals()
    ...
    def _get_observations(self) -> dict[str, torch.Tensor]:
        ...
        return {"policy_state": policy_state, "policy_image": policy_image}
