...
DEFAULT_EXPERIMENT_DIR = "/home/wei/End_to_end/logs/rsl_rl/quadcopter_obstacles_camera"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate a trained camera-depth quadcopter obstacles policy.")
    parser.add_argument("--task", type=str, default="Isaac-Quadcopter-Obstacles-Camera-v0")
    parser.add_argument("--num_envs", type=int, default=128)
    parser.add_argument("--steps", type=int, default=None)
    parser.add_argument("--model", type=str, default=None)
    parser.add_argument("--metrics_out", type=str, default="/home/wei/End_to_end/logs/rsl_rl/quadcopter_obstacles_camera/eval_camera_metrics.txt")
    ...
    try:
        ...
        import quadcopter_obstacles_camera  # noqa: F401
        from quadcopter_obstacles_camera.config_utils import apply_env_cfg_dir
        log("imports_after_app_ok")

        def _find_latest_model(experiment_dir: str) -> str:
            if not os.path.isdir(experiment_dir):
                raise ValueError(f"Experiment directory does not exist: {experiment_dir}")

            def checkpoint_sort_key(name: str) -> tuple[int, int]:
                if name == "model_latest.pt":
                    return (1, 0)
                match = re.fullmatch(r"model_(\d+)\.pt", name)
                return (0, int(match.group(1))) if match else (-1, -1)

            run_dirs = [entry.path for entry in os.scandir(experiment_dir) if entry.is_dir()]
            run_dirs.sort()
            for run_dir in reversed(run_dirs):
                checkpoints = [
                    name for name in os.listdir(run_dir) if re.fullmatch(r"model_(?:\d+|latest)\.pt", name)
                ]
                if not checkpoints:
                    continue
                checkpoints.sort(key=checkpoint_sort_key)
                return os.path.join(run_dir, checkpoints[-1])
            raise ValueError(f"No model_*.pt checkpoints found under: {experiment_dir}")
        ...
        model_path = args.model or _find_latest_model(DEFAULT_EXPERIMENT_DIR)
