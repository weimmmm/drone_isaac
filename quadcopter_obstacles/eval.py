...
DEFAULT_MODEL_PATH = (
    "/home/wei/End_to_end/logs/rsl_rl/quadcopter_obstacles_v5/2026-03-10_20-39-49/model_600.pt"
)
DEFAULT_METRICS_PATH = "/tmp/eval_metrics_model600.txt"

for path in (PROJECT_ROOT, os.path.join(PROJECT_ROOT, "rsl_rl")):
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate a trained quadcopter obstacles policy.")
    parser.add_argument("--task", type=str, default="Isaac-Quadcopter-Obstacles-v0")
    parser.add_argument("--num_envs", type=int, default=128)
    parser.add_argument("--steps", type=int, default=2000)
    parser.add_argument("--model", type=str, default=DEFAULT_MODEL_PATH)
    parser.add_argument("--metrics_out", type=str, default=DEFAULT_METRICS_PATH)
    ...
    try:
        ...
        import quadcopter_obstacles  # noqa: F401
        ...
        with torch.inference_mode():
            for step in range(args.steps):
                ...
                lin_vel_w = env.unwrapped._robot.data.root_lin_vel_w
                ...
                target_world = env.unwrapped._get_target_world()
                final_target_distance = torch.linalg.norm(
                    target_world - env.unwrapped._robot.data.root_pos_w, dim=1
                )
                final_obstacle_distance = env.unwrapped._compute_min_obstacle_distance()
        ...
        if final_obstacle_distance is not None:
            log(f"final_min_obstacle_distance_mean={final_obstacle_distance.mean().item():.4f}")
            log(f"final_min_obstacle_distance_median={final_obstacle_distance.median().item():.4f}")
