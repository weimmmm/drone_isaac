...
class QuadcopterObstaclesRefinerEnvCfg(QuadcopterObstaclesEnvCfg):
    ...
    depth_camera_offset_convention: str = "world"
    # 是否加载 teacher 网络。
...
class QuadcopterObstaclesRefinerEnv(QuadcopterObstaclesEnv):
                ...
                "policy": gym.spaces.Dict(
                    {
                        ...
                        "policy_image": gym.spaces.Box(
                            low=0.0,
                            high=1.0,
                            shape=(1, self.cfg.depth_camera_height, self.cfg.depth_camera_width),
                            dtype=float,
                        ),
                    }
                ),
    ...
    def _get_policy_image(self) -> torch.Tensor:
        ...
        self._debug("[DEBUG][refiner.env] fetching depth frames")
        depth_frames = self.get_record_depth_frames().unsqueeze(1)
        self._cached_policy_image = depth_frames
        self._last_image_step = self.common_step_counter
        self._debug("[DEBUG][refiner.env] depth frames ready")
        return depth_frames

    def get_record_depth_frames(self) -> torch.Tensor:
        """读取 tiled 相机的深度图，并归一化到 [0, 1]。"""
        if not self._built:
            self._build()
    ...
    def _get_observations(self) -> dict[str, torch.Tensor]:
        ...
        if self.cfg.enable_teacher_metrics:
            ...
            self._teacher_actions_valid = False
        # student 最终只看到“可部署低维状态 + 深度图”。
        policy_state = base_policy_obs[:, : self._policy_state_dim]
        policy_image = self._get_policy_image()
