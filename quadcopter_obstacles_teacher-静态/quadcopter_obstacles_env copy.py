...
# 最接近原版，                      
# Learning iteration 307/5000                       
#                        Computation: 63593 steps/s (collection: 5.062s, learning 0.091s)
#              Mean action noise std: 0.90
#           Mean value_function loss: 3.1339
#                Mean surrogate loss: -0.0005
#                  Mean entropy loss: 3.9166
#                        Mean reward: 126.76
#                Mean episode length: 146.91
#        Episode_Reward/vel_tracking: 0.0019
#             Episode_Reward/ang_vel: -0.0025
#  Episode_Reward/distance_to_target: 0.1668
#     Episode_Reward/target_velocity: 0.1494
#        Episode_Reward/target_bonus: 1.0821
#  Episode_Reward/obstacle_proximity: -0.0188
#            Episode_Reward/progress: 0.0112
#     Metrics/avg_vel_tracking_error: 0.0371
#           Episode_Termination/died: 66.1875
#       Episode_Termination/time_out: 0.0000
#               Metrics/success_rate: 0.9739
# 但是无人机活动范围是10*10的
...
class QuadcopterObstaclesEnvCfg(DirectRLEnvCfg):
    ...
    scene: InteractiveSceneCfg = InteractiveSceneCfg(
        ...
        env_spacing=20.0,  # 环境间距增大（避免障碍物跨环境干扰）
    ...
    obstacle_spawn_range = 8.0  # 障碍物生成范围（米）
    ...
    # 目标点详细配置
    target_spawn_range = 5.0  # 目标点采样半宽（xy会在[-range, range]内采样）
...
class QuadcopterObstaclesEnv(DirectRLEnv):
    ...
    def _randomize_obstacles(self, env_ids: torch.Tensor):
        ...
        min_radius = self.cfg.obstacle_safe_zone + self.cfg.obstacle_radius + 0.2
        max_radius = self.cfg.obstacle_spawn_range
        ...
        for obstacle_idx in range(self.cfg.num_obstacles):
            ...
            for _ in range(32):
                angle = torch.empty(num_envs_to_reset, device=self.device).uniform_(0.0, 2.0 * torch.pi)
                radius = torch.empty(num_envs_to_reset, device=self.device).uniform_(min_radius, max_radius)
                candidate_xy = torch.stack([radius * torch.cos(angle), radius * torch.sin(angle)], dim=1)

                if obstacle_idx == 0:
                    min_dist = torch.full((num_envs_to_reset,), float("inf"), device=self.device)
                    valid_mask = torch.ones(num_envs_to_reset, dtype=torch.bool, device=self.device)
                else:
                ...
                if chosen_xy is None:
                    ...
                    chosen_score = min_dist
                else:
                    better_mask = min_dist > chosen_score
                    chosen_xy[better_mask] = candidate_xy[better_mask]
                    chosen_score[better_mask] = min_dist[better_mask]
    ...
    def _randomize_targets(self, env_ids: torch.Tensor):
        ...
        clearance = self.cfg.obstacle_radius + self.cfg.target_reach_threshold
