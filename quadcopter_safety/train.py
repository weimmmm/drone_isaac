...
parser = argparse.ArgumentParser(description="Train the phase-1 quadcopter safety command policy.")
...
parser.add_argument("--task", type=str, default="Isaac-Quadcopter-Safety-v0")
...
import quadcopter_safety  # noqa: F401
...
def main():
    ...
    if args_cli.num_envs is not None and args_cli.num_envs > 32:
        print(
            "[WARN] Safety stage runs one shared Isaac world with one depth camera per robot. "
            "Large num_envs values can exhaust GPU memory quickly.",
            flush=True,
        )

    if args_cli.num_envs is not None:
        env_cfg.scene.num_envs = args_cli.num_envs
    if not getattr(args_cli, "headless", False) and getattr(env_cfg, "show_depth_preview", False):
        if env_cfg.scene.num_envs != 1:
            print(
                "[INFO] GUI depth preview mode forces num_envs=1 to avoid RTX out-of-memory "
                "and to show a single UAV front depth view clearly.",
                flush=True,
            )
    ...
    print("[DEBUG][SafetyTrain] creating gym env", flush=True)
    env = gym.make(args_cli.task, cfg=env_cfg, render_mode="rgb_array" if args_cli.video else None)
    print("[DEBUG][SafetyTrain] gym env created", flush=True)
    ...
    print("[DEBUG][SafetyTrain] wrapping env for RSL-RL", flush=True)
    env = RslRlVecEnvWrapper(env, clip_actions=agent_cfg.clip_actions)
    print("[DEBUG][SafetyTrain] wrapped env for RSL-RL", flush=True)
    print("[DEBUG][SafetyTrain] constructing OnPolicyRunner", flush=True)
    runner = OnPolicyRunner(env, class_to_dict(agent_cfg), log_dir=log_dir, device=agent_cfg.device)
    print("[DEBUG][SafetyTrain] OnPolicyRunner constructed", flush=True)
    ...
    print("[DEBUG][SafetyTrain] starting runner.learn()", flush=True)
    runner.learn(num_learning_iterations=agent_cfg.max_iterations, init_at_random_ep_len=True)
    print("[DEBUG][SafetyTrain] runner.learn() returned", flush=True)
    env.close()
